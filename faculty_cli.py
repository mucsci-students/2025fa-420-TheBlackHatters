...
def daysAndTimes():
    ...
    print("\033[31mNOTE: For the following prompts, if a faculty is available two or more different times in one day, separate the times by commas:\033[0m")
    #print("")
...
def editFacultyCLI(Faculty):
    ...
    if FacultyModel.Faculty.facCheck(Faculty, faculty_to_edit)==False:
        ...
        if "credit range" in things_to_edit:
            ...
            credit_range = {'credit range': str(min_credits) + "-" + str(max_credits)}
        else:
            credit_range = {'credit range': previous_faculty.get('credit range')}
        ...
        if "coursepreferences" in things_to_edit:
            ...
            if len(temp_preferences) == 0:
                ...
                while i < len(temp_preferences):
                    print("Enter the course weight (1-5) for " + temp_preferences[i] + " (Default is 3):")
                    course_weight[i] = input().replace(" ", "")
                    try:
                        if 1 <= int(course_weight[i]) <= 5:
                            course_weight[i] = int(course_weight[i])
            ...
            course_weight = {'course weight': previous_faculty.get('course weight')}
            preferences = {'preferences': previous_faculty.get('preferences')}
        ...
        edited_faculty = {faculty_name, full_or_part_time, unique_course_limit, time_available, preferences, course_weight, courses_taught, credit_range}
