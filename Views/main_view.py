...
def dataFacultyLeft(frame, controller, refresh, facultyData=None, app_instance=None):
    ...
    ctk.CTkButton(
        container, text="Add", width=120, height=20, command=lambda: onAdd()
    ...
    for faculty in sorted_fac:
        ...
        ctk.CTkLabel(
            rowFrame, text=facName, font=("Arial", 14, "bold"), anchor="w"
        ...
        ctk.CTkButton(
            ...
            width=30,
            height=20,
            command=lambda n=facName: onDelete(n),
        ...
        ctk.CTkButton(
            ...
            width=30,
            height=20,
            command=lambda f=faculty: onEdit(f),
        ).pack(side="left", padx=5)
...
def dataFacultyRight(frame, controller, refresh, data=None, app_instance=None):
    ...
    ctk.CTkLabel(
        rowName, text="Name:", anchor="w", font=("Arial", headingfont, "bold")
    ).pack(side="left", padx=(0,5), pady=(0, 2))

    # this is for and entry, this is where the user can write things in
    nameEntry = ctk.CTkEntry(
        rowName, placeholder_text="E.g: Hobbs", width=350, font=("Arial", textfont)
    )
    nameEntry.pack(side="left", fill="x", expand=False, padx=5)
    ...
    minEntry.pack(side="left", expand=False, padx=5)

    rowCredits = ctk.CTkFrame(frame, fg_color="transparent")
    rowCredits.pack(fill="x", pady=0, padx=5, expand=True)

    # Dropdown menu and label for maximum credits
    ctk.CTkLabel(rowCredits, text="Max Credits:", width=120, justify="left", anchor="w", font=("Arial", headingfont, "bold")).pack(
        side="left", padx=0, expand=False
    )
    ...
    maxEntry.pack(side="left", expand=False, padx=5)
    ...
    uniqueEntry.pack(side="left", fill="x", expand=False, padx=5)
    ...
    for day in days:
        ...
        dayEntry.pack(side="left", fill="x", expand=False)
    ...
    def update_course_dropdowns(*args):
        ...
        for var, dropdown in course_vars:
            current = var.get()
            available = [
                c for c in all_courses if c not in selected_courses or c == current
            ]
    ...
    ).pack(anchor="w", padx=0, pady=(2, 5))

    ctk.CTkLabel(
        rowCourse,
        text="(Set name to None to remove. Maximum 3 entries)",
        anchor="w",
        font=("Arial", subfont, "bold"),
        text_color="cyan",
        justify="left",
    ...
    ctk.CTkButton(
        ...
        height=20,
        font=("Arial", textfont, "bold"),
        command=lambda: preference_bar_creation("None", 5),
    ).pack(side=ctk.LEFT, padx=0)
    ...
    def preference_bar_creation(course_name, weight):
        ...
        # Weight dropdown
        weight_dropdown = ctk.CTkOptionMenu(
            courseRow, 
            width=65, 
            font=("Arial", textfont, "bold"),
    ...
    def update_room_dropdowns(*args):
        ...
        for var, dropdown in room_vars:
            ...
            available = [
                r for r in room_ids if r not in selected_rooms or r == current_value
            ]
    ...
    def room_bar_creation(room_name, weight):
        ...
        room_dropdown = ctk.CTkOptionMenu(
            roomRow,
            variable=room_var,
            values=values_list,
            width=65,
    ...
    def update_lab_dropdowns(*args):
        ...
        for var, dropdown in lab_vars:
            ...
            # Build available list without duplicating "None"
            available = [
                r for r in lab_ids if r not in selected_labs or r == current_value
            ]
    ...
    ctk.CTkButton(
        ...
        height=20,
        font=("Arial", textfont, "bold"),
        command=lambda: lab_bar_creation("None", 5),
    ).pack(side=ctk.LEFT, padx=0)
    ...
    def lab_bar_creation(lab, weight):
        ...
        dropdown = ctk.CTkOptionMenu(
            labRow, variable=lab_var, values=values_list, width=65, font=("Arial", textfont, "bold")
        )
        ...
        labPreferences[lab] = {"lab_var": lab_var, "weight_dropdown": weight_dropdown}
...
def dataRoomLeft(frame, controller, refresh, data=None, app_instance=None):
    ...
    for room in controller.listRooms():
        ...
        ctk.CTkButton(
            rowFrame, text="Edit", width=30, height=20, command=lambda r=room: onEdit(r)
        ).pack(side="left", padx=5)
...
def dataLabsRight(frame, controller, refresh, data=None, app_instance=None):
    ...
    nameEntry = ctk.CTkEntry(
        rowName, width=250, placeholder_text="E.g: Mac", font=("Arial", textfont, "bold")
    )
    nameEntry.grid(row=0, column=1, sticky="ew", padx=5)
...
def dataCoursesRight(frame, controller, refresh, data=None, app_instance=None):
    ...
    entry_id.pack(side="left", fill="x", expand=False)
    ...
    entry_cr.pack(side="left", fill="x", expand=False, padx=5)
        ...
        def update_dropdowns(*args):
            ...
            for var, menu in dropdown_menus:
                ...
                available = [
                    item
                    for item in items
                    if item not in selected_values or item == current_value
                ]
                available.insert(0, "None")
    ...
    room_vars = dropdown_section(
        frame, "Rooms", all_rooms, course.get("room") if course else []
    )
    lab_vars = dropdown_section(
        frame, "Labs", all_labs, course.get("lab") if course else []
    )
    conflict_vars = dropdown_section(
        frame,
        "Course Conflicts",
        all_courses,
        course.get("conflicts") if course else [],
    )
    faculty_vars = dropdown_section(
        frame, "Faculty", all_faculty, course.get("faculty") if course else []
    )
    ...
    def onSave():
        ...
        if course:
            ...
            error = controller.editCourse(
                course.get("course_id"), new_course, refresh, target_index=target_index
            )
            if not error and app_instance:
                # Record the edit action after successful edit
                app_instance.record_course_edit(
                    old_course_data, new_course, target_index
                )
        else:
...
def defaultScheduleViewer(
    ...
    tableFrame = ctk.CTkFrame(
        schFrame, fg_color="transparent", height=300
...
def roomLabsScheduleViewer(
    frame, schedulesInstance, pathEntaryVar, idx, numOfSch, order, createDropdown
):
    # for each of the schedules we create a frame to put schedule in
        ...
        for room, classes in schedule_dict.items():
            ...
            for cls in classes:
                ...
                for meeting in meetings:
                    ...
                    y = (
                        start.hour + start.minute / 60 - startHour
                    ) * hourHeight + offset
...
class SchedulerApp(ctk.CTk):
    ...
    def record_lab_addition(self, lab_name):
        ...
        self.record_action("lab_addition", {"lab_name": lab_name, "name": lab_name})

    def record_lab_edit(self, old_name, new_name):
        """Record a lab edit for undo/redo"""
        self.record_action(
            "lab_edit", {"old_name": old_name, "new_name": new_name, "name": new_name}
        )

    def record_lab_deletion(self, lab_name):
        """Record a lab deletion for undo/redo"""
        self.record_action("lab_deletion", {"lab_name": lab_name, "name": lab_name})

    def record_course_addition(self, course_data):
        """Record a course addition for undo/redo"""
        self.record_action(
            "course_addition",
            {"course_data": course_data, "name": course_data.get("course_id", "")},
        )
    ...
    def record_faculty_addition(self, faculty_data):
        ...
        self.record_action(
            "faculty_addition",
            {"faculty_data": faculty_data, "name": faculty_data.get("name", "")},
        )
    ...
    def record_room_addition(self, room_name):
        ...
        self.record_action("room_addition", {"room_name": room_name, "name": room_name})

    def record_room_edit(self, old_name, new_name):
        """Record a room edit for undo/redo"""
        self.record_action(
            "room_edit", {"old_name": old_name, "new_name": new_name, "name": new_name}
        )
    ...
    def _undo_course_edit(self, action):
        ...
        courseCtr.editCourse(
            course_id,
            old_course_data,
            refresh=None,
            target_index=action["data"].get("target_index"),
        )
        self.refresh("ConfigPage")
    ...
    def createConfigPage(self, parent=None, data=None):
        ...
        pathEntry = ctk.CTkEntry(
            importFrame, state="readonly", textvariable=self.configPath, width=500
        )
        ...
        self.createTwoColumn(
            tabview.tab("Faculty"),
            lambda frame: dataFacultyLeft(
                frame, facultyCtr, self.refresh, app_instance=self
            ),
            lambda frame: dataFacultyRight(
                frame, facultyCtr, self.refresh, faculty_data, app_instance=self
            ),
            # Holding this
        )
        ...
        self.createTwoColumn(
            tabview.tab("Courses"),
            lambda frame: dataCoursesLeft(
                frame, courseCtr, self.refresh, app_instance=self
            ),
            lambda frame: dataCoursesRight(
                frame, courseCtr, self.refresh, course_data, app_instance=self
            ),
        )
        ...
        self.createTwoColumn(
            tabview.tab("Labs"),
            lambda frame: dataLabsLeft(
                frame, labCtr, self.refresh, data, app_instance=self
            ),
            lambda frame: dataLabsRight(
                frame,
                labCtr,
                self.refresh,
                data if isinstance(data, str) else None,
        ...
        ctk.CTkLabel(
            chatbot_bar, text="Scheduler Assistant:", font=("Arial", 18, "bold")
        ).pack(side="left", padx=10)
        ...
        chatbot_button = ctk.CTkButton(
            chatbot_bar, text="Send", width=100, command=lambda: handle_chat()
        )
    ...
    def createViewSchedulePage(self, parent=None, schedules=None):
        ...
        if self.schedulesImportedPath.get() == "":
            self.schedulesImportedPath.set(
                "View your imported or generated schedules here!"
            )
        ...
        importBtn = ctk.CTkButton(
            importFrame, text="Import Schedules", width=150, command=onImport
        )
        importBtn.pack(side="left", padx=(0, 10))

        pathEntry = ctk.CTkEntry(
            importFrame, state="readonly", textvariable=self.schedulesImportedPath
        )
        pathEntry.pack(side="left", padx=(0, 10), fill="x", expand=True)
        ...
        nextBTN = ctk.CTkButton(
            buttons_frame, text="Next", width=150, command=lambda: changeSchedule(1)
        )
        ...
        def updateButtonStates():
            if self.numOfSch <= 1:
                ...
                previousBTN.configure(
                    state="normal" if self.schIndex > 0 else "disabled"
                )
                nextBTN.configure(
                    state="normal" if self.schIndex < self.numOfSch - 1 else "disabled"
                )
        ...
        ctk.CTkLabel(buttons_frame, text=f"of {self.numOfSch}").pack(
            side="left", padx=(5, 5)
        )
    ...
    def createSchedulerPage(self, parent=None):
        ...
        for opt in optimizeList:
            ...
            checkbox = ctk.CTkCheckBox(
                optionsFrame, text=opt.replace("_", " ").title(), variable=var
            )
        ...
        def onGenerate():
            ...
            if limit_value.isdigit():
                ...
                def progress_callback(current_step, total_steps):
                    ...
                    if has_opt and current_step == 1:
                        ...
                        status_label.configure(
                            text=f"Generating schedule {schedule_num}/{limit_int}"
                        )
                ...
                try:
                    ...
                    if (
                        self.configPath.get() == ""
                        or self.configPath.get() == ".json"
                        or "Import Config File" in self.configPath.get()
                    ):
                        raise ValueError(
                            "No configuration file loaded. Please import a config file first!"
                        )
                    # This will block the UI but show progress updates
                    self.schedulesImported = generateSchedulesBtn(
                        limit_int, selectedOpts, progress_callback
                    )
                    ...
                    if "No configuration file" in str(e):
                        status_label.configure(
                            text="Error: No configuration file loaded!"
                        )
        ...
        genBtn = ctk.CTkButton(
            container, text="Generate Schedules", width=150, command=onGenerate
        )
