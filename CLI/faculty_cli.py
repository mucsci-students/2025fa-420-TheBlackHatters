...
def daysAndTimes():
    ...
    print(
        "\033[31mNOTE: For the following prompts, if a faculty is available two or more different times in one day,\033[0m"
    )
    ...
    for day in days:
        print(
            f'When are they available on {day}? (Leave blank for 9:00-5:00, type "N/A" if they are not available)'
        )
...
def addFacultyCLI(Faculty):
    ...
    print(
        "Enter the maximum credits a faculty can teach per semester (Default is "
        + str(max_teachable_credits)
        + "):"
    )
    max_credits = input()
    try:
        if int(max_credits) > max_teachable_credits or int(max_credits) < int(
            min_credits
        ):
    ...
    print(
        "What are their course preferences? (Separate courses with commas, maximum of 3)"
    )
    ...
    if len(course_preferences) > 0:
        ...
        while i < len(course_preferences):
            print(
                "Enter the course weight (0-10) for "
                + str(course_preferences[i])
                + " (Default is 5, 0 means no preference):"
            )
            ...
            i += 1
        course_preferences = {
            course_result[i]: course_result[i + 1]
            for i in range(0, len(course_result), 2)
        }

    # Gets the faculty room preferences
    print(
        "What are their room preferences? (Separate courses with commas, maximum of 3)"
    )
    ...
    if len(room_preferences) > 0:
        ...
        while i < len(room_preferences):
            print(
                "Enter the room weight (0-10) for "
                + str(room_preferences[i])
                + " (Default is 5, 0 means no preference):"
            )
            ...
            i += 1
        room_preferences = {
            room_result[i]: room_result[i + 1] for i in range(0, len(room_result), 2)
        }

    # Gets the faculty lab preferences
    print(
        "What are their lab preferences? (Separate courses with commas, maximum of 3)"
    )
    ...
    if len(lab_preferences) > 0:
        ...
        while i < len(lab_preferences):
            print(
                "Enter the lab weight (0-10) for "
                + str(lab_preferences[i])
                + " (Default is 5, 0 means no preference):"
            )
            ...
            i += 1
        lab_preferences = {
            lab_result[i]: lab_result[i + 1] for i in range(0, len(lab_result), 2)
        }
...
def editFacultyCLI(Faculty):
    ...
    if FacultyModel.Faculty.facCheck(Faculty, faculty_to_edit) == False:
        ...
        if "credits" in things_to_edit:
            print(
                'Are they full-time or adjunct? (Enter "full" or "adjunct") (Default: full)'
            )
            ...
            # Gets the minimum credits the faculty can teach
            print(
                "Enter the minimum credits a faculty can teach per semester (default is 0):"
            )
            minimum_credits = input()
            try:
                if int(minimum_credits) < 0 or int(minimum_credits) > int(
                    max_teachable_credits
                ):
                    minimum_credits = 0
            except ValueError:
                minimum_credits = 0
            ...
            try:
                if int(maximum_credits) > max_teachable_credits or int(
                    maximum_credits
                ) < int(maximum_credits):
        ...
        if "coursepreferences" in things_to_edit:
            ...
            course_preferences = list(
                filter(None, input().lower().replace(" ", "").split(","))
            )
            if len(course_preferences) > 2:
                course_preferences = course_preferences[:3]
            if len(course_preferences) == 0:
        ...
        if "roompreferences" in things_to_edit:
            ...
            if len(room_preferences) == 0:
                ...
                while i < len(room_preferences):
                    ...
                    i += 1
                room_preferences = {
                    room_result[i]: room_result[i + 1]
                    for i in range(0, len(room_result), 2)
                }
        ...
        if "labpreferences" in things_to_edit:
            ...
            if len(lab_preferences) == 0:
                ...
                while i < len(lab_preferences):
                    ...
                    i += 1
                lab_preferences = {
                    lab_result[i]: lab_result[i + 1]
                    for i in range(0, len(lab_result), 2)
                }
...
def removeFacultyCLI(Faculty):
    ...
    if (
        ...
        print(
            "Are you sure you want to delete this faculty? This cannot be undone. (y/n)"
        )
